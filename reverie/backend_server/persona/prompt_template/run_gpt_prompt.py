...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    cr = int(gpt_response.strip().lower().split("am")[0][-1])
    return cr
...
def run_gpt_prompt_daily_plan(persona,
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    for i in _cr:
      if i[-1].isdigit() and int(i[-1]) == cur_idx+1:
        i = i[:-1].strip()
        if i:
          if i[-1] == "." or i[-1] == ",":
            cr += [i[:-1].strip()]
...
def run_gpt_prompt_generate_hourly_schedule(persona,
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    if cr.startswith("[(ID"):
        cr = cr.split('is')[-1].strip()
    assert cr.split()[0][-3:] == "ing"
    if cr[-1] == ".":
      cr = cr[:-1]
    return cr
...
def run_gpt_prompt_task_decomp(persona,
  ...
  def __func_clean_up(gpt_response, prompt=""):
    ...
    cr = []
    need_idx = 0
    for count, i in enumerate(temp[0].split()):
      if "ing" in i:
        need_idx = count
        break
    for count, i in enumerate(temp):
      if i[0].isdigit() and need_idx:
        _cr += [" ".join([j.strip () for j in i.split(" ")][need_idx:])]
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False):
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "/data1/LLM-exp/generative_agents_remix/reverie/backend_server/persona/prompt_template/v3_ChatGPT/generate_obj_event_v1.txt" ########
  ...
  output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
                                          __chat_func_validate, __chat_func_clean_up, gpt_param, True)
  if output != False:
    return output, [output, prompt, gpt_param, prompt_input, fail_safe]
  else:
